...
3. 雲端運算與物聯網代表了網際網路應用的最新演進方向，理解它們的服務模式與部署模式，能幫助看懂新聞中常見的科技名詞。
...
> **課堂重點：三種計算方法的判斷口訣**——看題目**問的是什麼**：問「這個 IP 屬於哪個網段」→ 用方法一（AND 運算）；問「要分成幾個子網路，遮罩怎麼設」→ 用方法二（先算子網路數要幾個位元）；問「要容納幾台主機，遮罩怎麼設」→ 用方法三（先算主機數要幾個位元，注意要扣 2）。務必分辨清楚題目問的究竟是「子網路數」還是「主機數」，這是最容易混淆、也是失分最多的地方。
...
> **課堂重點（呼應學習評量第 6 題）**：**中文網域名稱**（例如「台灣.tw」這類中文網址）的註冊服務在台灣是由 **TWNIC（台灣網路資訊中心）** 負責管理；至於一般網域名稱的整體協調管理，則有 ICANN、IETF 等國際組織參與制定標準，只需要記得「**.tw** 相關的網域註冊找 TWNIC」即可。
...
> **提醒**：這一題的子遮罩、IP 位址數值較多，建議實際逐一將每個選項轉成二進位、套用遮罩進行 AND 運算演練，比單純背答案更能建立扎實的計算能力。一般化的判斷方法為：把子遮罩與各選項的 IP 位址都轉成二進位，逐位元做 AND 運算求出各自的網路位址，再比較哪一個選項算出來的網路位址，與其餘選項不同，那一個就是答案。
